...
#### 4.3.1 存储与实例参数
- 引擎与字符集：全库 InnoDB，utf8mb4/utf8mb4_unicode_ci，隔离级别 REPEATABLE READ。
- 关键参数（覆盖默认）：innodb_buffer_pool_size=物理内存 70%-80%；innodb_log_file_size≈1G；innodb_flush_log_at_trx_commit=1、sync_binlog=1 保证崩溃恢复；innodb_flush_method=O_DIRECT 减少双写放大；slow_query_log=ON，long_query_time=1s；performance_schema=ON、sys 库启用。

#### 4.3.2 表与列的物理定义
- 主键全部自增/定长整数，避免随机 UUID 碎片；金额列 DECIMAL(10,2)/(12,2)；age/quantity 等列使用 CHECK 约束；批次号、手机号使用 UNIQUE 约束。
- 全表增加 created_at，部分表增加 updated_at，便于增量抽取与审计。

#### 4.3.3 索引与访问路径（B+ 树）
- InnoDB 聚簇索引+二级索引均为 B+ 树；热点点查依赖范围/前缀匹配，自适应哈希由 InnoDB 自动启用。
- 已建二级索引（见 sql/init.sql 第八部分）：
...
- 设计原则：订单类按日期+主键覆盖近期查询；批次表用(med_id, expiry_date) 覆盖先药品后效期的范围查询；明细表用(主表 ID, 药品 ID) 减少回表；客户表 phone/total_consume 支撑查重与榜单。
...
#### 4.3.5 日志、监控与慢 SQL 治理
- 慢日志：long_query_time=1s，log_queries_not_using_indexes=ON，周度审查 top N 慢语句并 EXPLAIN 优化。
- 性能诊断：performance_schema + sys 库查看 wait/lock/hot tables；INNODB_METRICS 监控缓冲池命中率与行锁等待。
- 运行监控：Prometheus mysqld-exporter 拉取 QPS、事务、redo、buffer pool 命中率，Grafana 报警阈值（响应时间>100ms、行锁等待飙升）。

#### 4.3.6 备份与恢复
- 备份：每日全量（可用 mysqldump/xtrabackup），每小时增量 binlog 归档到对象存储；保留策略 30 天。
- 恢复演练：月度按“最近全量+增量 binlog”做沙箱恢复，校验行数与关键报表；恢复脚本固化并版本化。
- 高可用与读写分离：主库写、只读从库提供查询与报表，半同步或异步复制；定期切换演练确保可用。
